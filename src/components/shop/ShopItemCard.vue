  ...
  <article v-if="item.is_active" class="shop-card" :class="{ 'has-discount': hasDiscount }">
    <router-link :to="getServiceDetailPath(item)" class="media" :aria-label="itemName">
      ...
      <div class="top-badges">
        <span v-if="item.category" class="category-badge">{{ categoryName }}</span>
...
const categoryName = computed(() => {
  ...
  const normalizedId = props.item.category
      .split(" ")
      .map(word => word.charAt(0).toUpperCase() + word.slice(1).toLowerCase())
      .join("");
  const key = `shopCategory${normalizedId}`;
  const translated = t(key);
  return translated === key ? props.item.category : translated;
});
...
@media (hover:hover) { .shop-card:hover { transform:translateY(var(--hover-card)); border-color:rgba(200,178,115,.42); box-shadow:0 20px 58px rgba(0,0,0,.32); } }
...
.item-image { object-fit:cover; opacity:0; transform:scale(1); filter:saturate(.86) contrast(1.04); transition:opacity var(--motion-slow) var(--ease-standard),transform var(--motion-slow) var(--ease-standard),filter var(--motion-base) var(--ease-standard); }
.image-shell.loaded .item-image { opacity:1; }
@media (hover:hover) { .shop-card:hover .item-image { transform:scale(var(--hover-image-scale)); filter:saturate(1.02) contrast(1.07); } }
...
.category-badge { max-width:75%; overflow:hidden; text-overflow:ellipsis; padding:6px 10px; border:1px solid rgba(255,255,255,.13); border-radius:var(--radius-pill); background:rgba(7,9,18,.68); color:#e8dfc4; font:600 10px/1 var(--font-ui); letter-spacing:1.2px; text-transform:uppercase; white-space:nowrap; }
.discount-badge { padding:7px 10px; border-radius:var(--radius-pill); background:var(--myst-gold); color:#11131c; font:800 12px/1 var(--font-ui); box-shadow:0 5px 18px rgba(200,178,115,.3); }
.feature-badges { left:14px; bottom:14px; }
.feature-badge { display:flex; align-items:center; gap:6px; padding:6px 9px; border:1px solid rgba(200,178,115,.18); border-radius:var(--radius-pill); background:rgba(7,9,18,.72); color:#ddd2b3; font-size:11px; }
.card-content { flex:1; display:flex; flex-direction:column; padding:20px; }
.card-heading h3 { margin:0; color:var(--myst-offwhite); font:700 clamp(20px,2vw,24px)/1.2 var(--font-display); }
...
.tooltip>span { position:absolute; right:0; bottom:calc(100% + 8px); width:min(230px,70vw); padding:10px 12px; visibility:hidden; opacity:0; transform:translateY(4px); border:1px solid rgba(200,178,115,.25); border-radius:var(--radius-md); background:#080b14; color:#e2e2e6; box-shadow:0 10px 30px #000; transition:opacity var(--motion-base) var(--ease-standard),transform var(--motion-base) var(--ease-standard),visibility var(--motion-base) var(--ease-standard); z-index:10; }
...
.price-label { display:block; margin-bottom:4px; color:#707686; font:600 9px/1 var(--font-ui); letter-spacing:1.4px; text-transform:uppercase; }
.prices { display:flex; align-items:baseline; flex-wrap:wrap; gap:8px; }
.original-price { display:flex; align-items:center; gap:3px; color:#6f7481; font:500 12px var(--font-ui); text-decoration:line-through; }
.current-price { display:flex; align-items:center; gap:5px; color:var(--myst-gold); font:800 22px/1.2 var(--font-ui); }
.prices svg { width:18px; height:18px; }
.original-price svg { width:12px; height:12px; }
.actions { display:flex; gap:8px; }
.details-button,.purchase-button { min-height:44px; border-radius:var(--radius-md); transition:background-color var(--motion-base) var(--ease-standard),border-color var(--motion-base) var(--ease-standard),color var(--motion-base) var(--ease-standard),transform var(--motion-base) var(--ease-standard); }
...
.purchase-button { min-width:126px; display:flex; align-items:center; justify-content:center; gap:8px; padding:0 18px; border:1px solid rgba(200,178,115,.45); background:rgba(200,178,115,.1); color:var(--myst-gold); cursor:pointer; font:700 11px var(--font-ui); letter-spacing:1.1px; text-transform:uppercase; }
.details-button:hover,.purchase-button:hover:not(:disabled) { transform:translateY(var(--hover-control)); }
