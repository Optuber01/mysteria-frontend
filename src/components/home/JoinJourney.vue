  ...
  >
    <div class="join-sticky">
      ...
      <header class="join-heading">
        <p>Enter Mysterria</p>
        <h2 id="join-title">Enter Mysterria.</h2>
        <span>01 — 03</span>
      </header>

      <p class="join-promise">Connect <span>→</span> Enter Mysteries <span>→</span> Find a recipe <span>→</span> Brew <span>→</span> Become</p>
      ...
      <div class="join-content">
        ...
        <aside class="connection-panel" aria-label="Connect to Mysterria">
          <p class="panel-label">Portal access · Java Edition</p>
...
const steps = [
  {
    id: 'connect', short: 'Connect', eyebrow: 'At the threshold', title: 'Find the server.',
...
<style scoped>
.join-story { --join-progress: 0; --scene-position: 50%; --scene-scale: 1.06; --heading-opacity: 1; position: relative; min-height: 300svh; color: #fcf9f2; background: #091718; }
...
.join-scenes img { width: 100%; height: 100%; object-fit: cover; object-position: var(--scene-position) 48%; filter: brightness(.67) saturate(.8) hue-rotate(8deg); transform: scale(var(--scene-scale)); transition: transform 950ms cubic-bezier(.22, 1, .36, 1), filter 800ms ease; }
.join-scenes::after { content: ''; z-index: 1; background: linear-gradient(90deg, rgba(5,17,19,.86) 0%, rgba(6,20,22,.5) 42%, rgba(5,17,19,.12) 76%), linear-gradient(0deg, rgba(5,17,19,.9), transparent 55%); }
.join-scenes__veil { z-index: 2; background: radial-gradient(ellipse at 52% 43%, rgba(121,204,186,.2), transparent 28%), linear-gradient(180deg, rgba(255,255,255,.025), transparent 26%); }
.join-scenes__grain { z-index: 3; opacity: .1; mix-blend-mode: soft-light; background-image: radial-gradient(rgba(255,255,255,.7) .5px, transparent .5px); background-size: 4px 4px; }
...
.join-heading h2 { max-width: 500px; margin: 13px 0 17px; font: 650 clamp(3.8rem, 6.2vw, 7rem)/.82 'IBM Plex Sans Condensed', sans-serif; letter-spacing: -.06em; text-wrap: balance; }
.join-heading > span { color: rgba(252,249,242,.46); }
.join-promise { position: absolute; z-index: 2; top: clamp(284px, 43vh, 410px); left: clamp(22px, 6vw, 96px); max-width: 560px; margin: 0; color: rgba(252,249,242,.68); font: 600 clamp(.58rem, .8vw, .7rem)/1.55 'IBM Plex Mono', monospace; letter-spacing: .08em; text-transform: uppercase; }.join-promise span { margin: 0 7px; color: #e6bd75; }
...
.connection-panel { position: absolute; left: 50%; bottom: 84px; width: min(1060px, calc(100% - 44px)); display: grid; grid-template-columns: .75fr 1.2fr 1.15fr auto; align-items: center; gap: 16px; padding: 13px 16px; border-top: 1px solid rgba(229,187,114,.72); border-bottom: 1px solid rgba(252,249,242,.16); background: rgba(5,18,20,.72); box-shadow: 0 20px 70px rgba(0,0,0,.25); backdrop-filter: blur(15px); transform: translateX(-50%); pointer-events: auto; }
...
.join-links { display: flex; gap: 8px; }.join-links a { min-height: 44px; display: flex; align-items: center; justify-content: center; gap: 5px; padding: 7px; border: 1px solid rgba(252,249,242,.14); border-radius: 4px; color: rgba(252,249,242,.8); font-size: .65rem; font-weight: 750; }.copy-feedback { display: none; }
...
@media (max-width: 760px) { .join-story { min-height: 285svh; }.join-sticky { min-height: 650px; }.join-heading { top: 82px; left: 17px; width: calc(100% - 34px); }.join-heading h2 { max-width: 380px; font-size: clamp(3.5rem, 16vw, 5.7rem); }.join-promise { top: 251px; left: 17px; width: calc(100% - 34px); font-size: .54rem; }.join-copy { z-index: 1; top: 305px; bottom: auto; left: 17px; width: calc(100% - 34px); transform: none; }.join-copy h3 { font-size: clamp(2.55rem, 11vw, 3.8rem); }.join-copy-enter-from { transform: translateY(16px); }.join-copy-leave-to { transform: translateY(-12px); }.connection-panel { right: auto; bottom: 77px; left: 17px; width: calc(100% - 34px); grid-template-columns: 1fr; gap: 6px; padding: 12px; transform: none; }.edition-note { display: none; }.join-steps { bottom: 8px; width: 100%; }.join-steps strong { display: none; }.join-scenes::after { background: linear-gradient(0deg, rgba(5,17,19,.88), rgba(5,17,19,.08) 72%), linear-gradient(90deg, rgba(5,17,19,.62), transparent); } }
