...
import type {BalanceDto, BalanceResponse} from "@/types/balance";
import type {ServiceDto, ServiceMarkdownDto, ServicePoint, ServiceResponse,} from "@/types/services";
    ...
    actions: {
        ...
        async initiatePurchase(itemId: string, amount: number = 1, recipientId?: string, selectedServer?: string) {
            ...
            try {
                ...
                const {t} = useI18n();
                console.error(t("errorPurchasing"), error);
                console.log(
                    "Error type:",
                    typeof error,
                ...
                if (error instanceof Error) {
                    ...
                    if (errorMessage.includes("Insufficient balance")) {
                        ...
                        errorMessage = t("errorServiceNotFoundOrUnavailable");
                    } else if (errorMessage.includes("permission")) {
                        errorMessage = t("errorInsufficientPermissions");
                    ...
                    show(t("errorUnknownPurchase"), {
                        type: "error",
                        duration: 6000,
                    });

                    setTimeout(() => {
                        show(t("errorDetailsInConsole"), {
                            type: "info",
                            duration: 4000,
                        });
                    }, 1000);
